...
class CartManager {
    ...
    async addToCart(cartId, productId) {
        ...
        const product = await this.findProductByCode(productId);
        if (!product) {
        ...
        if (productIndex !== -1) {
            ...
            carts[cartIndex].products.push({ code: product.code, quantity: 1, title: product.title, category: product.category, price: product.price }); 
        }
